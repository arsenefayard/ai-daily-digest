...
def fetch_history(repo, headers):
    api_url = f"https://api.github.com/repos/{repo}/contents/sport_history.json"
...
def push_to_github(data, history):
    ...
    try:
        push_file(repo, headers, "sport_today.json", json.dumps(data, ensure_ascii=False, indent=2), f"sport-digest: {date_str}")
        push_file(repo, headers, "sport_history.json", json.dumps(history, ensure_ascii=False, indent=2), f"sport-history: {date_str}")
        if os.path.exists("sport.html"):
            _h = open("sport.html", encoding="utf-8").read().replace("__PX_KEY_B64__", __import__("base64").b64encode(os.environ.get("PERPLEXITY_API_KEY","").encode()).decode())
            push_file(repo, headers, "sport.html", _h, "chore: update sport.html")
        return True
...
def get_news(history_context=""):
    ...
    print("🔍 Recherche des actualités sport...")
    history_section = f"\n\n{history_context}\n" if history_context else ""
    payload = {
        "model": "sonar",
        "messages": [
            {"role": "system", "content": f"""Tu es un assistant expert en actualités sportives mondiales.

Recherche sur le web les 5 actualités sportives les plus importantes publiées aujourd'hui ou dans les derniers jours.{history_section}
IMPORTANT : Tu dois TOUJOURS inclure au moins 1 actualité sur le tennis parmi les 5 news.
